    ...
    axios.post("http://localhost:8080/api/v1/events/updatestatus/" + status + "/" + eventId, { headers: headers })
      .then(res => {
        if (status == 0) {
          Swal.fire({
            title: "Thành công",
            text: 'Dừng dăng ký sự kiện thành công',
            icon: "success",
          }).then(() => {
            getEvents()
            // location.reload();
          ...
          Swal.fire({
            title: "Thành công",
            text: 'Mở đăng ký sự kiện thành công',
            icon: "success",
          }).then(() => {
            getEvents()
      ...
      .catch(e => {
        ...
        if (e.response.status === 400) {
          ...
          errMes = 'Cập nhật trạng thái sự kiện thất bại'
        }
        Swal.fire({
          title: "Thất bại",
          text: errMes,
  ...
  return (
    <Styles>
      <TextField style={{ paddingBottom: "2px" }}
        ...
        value={search}
      />
      <Popup onClose={getEvents} modal trigger={<Button color="primary" style={{ float: "right" }}>Thêm sự kiện</Button>}>
        {close =>
          <ModalCreateEvent
            close={close}
          />}
      ...
      <Card>
        <CardHeader color="primary">
          <h4 className={classes.cardTitleWhite}>Quản lý sự kiện</h4>
          <p className={classes.cardCategoryWhite}>
            Quản lý sự kiện, thêm sự kiện hoặc chỉnh sửa sự kiện của bảo tàng.
          </p>
        </CardHeader>
        <CardBody>
                      ...
                      <Button size="sm" color="danger"
                        onClick={() => {
                          ...
                          if (row.active == 'Đang mở đăng ký') {
                            temp = 'ngưng sự kiện ' + row.eventName + ' không cho đăng ký?'
                          } else {
                            temp = 'mở lại sự kiện ' + row.eventName + ' cho đăng ký?'
                          }
                          setConfirmDialog({
                            isOpen: true,
